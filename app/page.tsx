...
function NewCaseModal({ profile, profiles, customers, orderSystems, openCount, onClose, onCreated }: { profile: Profile; profiles: Profile[]; customers: NevCustomer[]; orderSystems: OrderSystem[]; openCount: number; onClose: () => void; onCreated: () => void }) {
  ...
  const [actionUser, setActionUser] = useState(() => profiles.find((person) => person.is_super_admin)?.user_id || ""); const [actionNote, setActionNote] = useState(""); const [files, setFiles] = useState<File[]>([]);
...
function CaseDetailsModal({ item, profile, profiles, onClose, onChanged }: { item: NevCase; profile: Profile; profiles: Profile[]; onClose: () => void; onChanged: () => void }) {
  ...
  const [actionRecipient, setActionRecipient] = useState(() => profiles.find((person) => person.is_super_admin)?.user_id || ""); const [actionNote, setActionNote] = useState(""); const [actionMood, setActionMood] = useState<CustomerMood | "">("");
  const [releaseNote, setReleaseNote] = useState(""); const [releaseTo, setReleaseTo] = useState(item.assigned_to || ""); const [releaseOptions, setReleaseOptions] = useState<ResolutionType[]>(item.available_resolutions || []);
  ...
  return <Modal title={item.customer_name} eyebrow={`${caseCode(item.case_number)} · ${stageLabels[item.workflow_stage]}`} onClose={onClose} wide><div className="details-layout details-layout--workflow"><div className="details-main">
    ...
    {canRelease && <section className="details-block action-card action-card--attention"><h3><ShieldCheck size={18} /> Clovis: libere as opções de conclusão</h3><p>Escolha tudo que Jean poderá apresentar ao cliente e para quem o atendimento voltará.</p><div className="solution-options">{selectableSolutions.map(([key, label]) => <label key={key} className={releaseOptions.includes(key) ? "is-selected" : ""}><input type="checkbox" checked={releaseOptions.includes(key)} onChange={(e) => setReleaseOptions((current) => e.target.checked ? [...current, key] : current.filter((value) => value !== key))} /><span>{label}</span></label>)}</div><div className="form-grid"><label className="field"><span>Quem falará com o cliente?</span><select value={releaseTo} onChange={(e) => setReleaseTo(e.target.value)}>{profiles.filter((person) => person.active).map((person) => <option key={person.user_id} value={person.user_id}>{person.full_name || person.email}</option>)}</select></label><label className="field field--wide"><span>Orientação para o atendente</span><textarea value={releaseNote} onChange={(e) => setReleaseNote(e.target.value)} rows={3} placeholder="Ex.: apresente as opções e confirme qual o cliente escolheu." /></label><button type="button" className="button button--success field--wide" onClick={release} disabled={busy || !releaseOptions.length}><CheckCircle2 size={17} /> Liberar e devolver para o atendente</button></div></section>}
