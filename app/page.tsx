...
import type {
  AuditEvent, CaseAttachment, CaseOrder, CaseStatus, CaseUpdate, NevCase, NevCustomer, NevTask,
...
function NewCaseModal({ profile, profiles, customers, orderSystems, onClose, onCreated }: { profile: Profile; profiles: Profile[]; customers: NevCustomer[]; orderSystems: OrderSystem[]; onClose: () => void; onCreated: () => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    const { data, error: createError } = await supabase.rpc("nev_create_case_with_orders", { p_customer_name: name.trim(), p_whatsapp: normalized, p_issue_type: issue, p_description: description.trim(), p_priority: priority, p_assigned_to: profile.user_id, p_orders: preparedOrders });
...
function Workspace({ profile, onSignOut }: { profile: Profile; onSignOut: () => void }) {
  const [tab, setTab] = useState<Tab>("dashboard"); const [cases, setCases] = useState<NevCase[]>([]); const [profiles, setProfiles] = useState<Profile[]>([]); const [customers, setCustomers] = useState<NevCustomer[]>([]); const [orderSystems, setOrderSystems] = useState<OrderSystem[]>([]); const [refunds, setRefunds] = useState<RefundPlan[]>([]); const [tasks, setTasks] = useState<NevTask[]>([]); const [audits, setAudits] = useState<AuditEvent[]>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState(""); const [newCase, setNewCase] = useState(false); const [newTask, setNewTask] = useState(false); const [selectedCase, setSelectedCase] = useState<NevCase | null>(null); const [selectedTask, setSelectedTask] = useState<NevTask | null>(null); const [payment, setPayment] = useState<RefundInstallment | null>(null); const [toast, setToast] = useState("");
  const loadAll = useCallback(async () => {
    const requests = [supabase.from("nev_cases").select("*").order("last_activity_at", { ascending: false }), supabase.from("nev_profiles").select("*").order("created_at"), supabase.from("nev_customers").select("*").order("created_at", { ascending: false }), supabase.from("nev_order_systems").select("*").eq("active", true).order("sort_order"), supabase.from("nev_refund_plans").select("*, nev_cases(id,case_number,customer_name,whatsapp), nev_refund_installments(*)").order("created_at", { ascending: false }), supabase.from("nev_tasks").select("*").order("updated_at", { ascending: false })];
    const [caseR, profileR, customerR, systemR, refundR, taskR] = await Promise.all(requests); const firstError = caseR.error || profileR.error || customerR.error || systemR.error || refundR.error || taskR.error;
    if (firstError) setError(firstError.message); else { setError(""); setCases((caseR.data || []) as NevCase[]); setProfiles((profileR.data || []) as Profile[]); setCustomers((customerR.data || []) as NevCustomer[]); setOrderSystems((systemR.data || []) as OrderSystem[]); setRefunds((refundR.data || []) as unknown as RefundPlan[]); setTasks((taskR.data || []) as NevTask[]); }
